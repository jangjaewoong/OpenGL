...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(800,800,"Graphics",NULL,NULL); // window 창을 생성하는 함수. 매개변수는 순서대로 넓이, 높이, 이름, 풀스크린 여부
    if(!window){
    ...
    glViewport(0,0,800,800); // OpenGL이 렌더링할 화면 영역 설정
    ...
    Texture backGround("resources/Textures/SunsetCity.jpeg", GL_TEXTURE_2D, GL_TEXTURE0, GL_UNSIGNED_BYTE);
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        backGround.Bind();
        VAO1.Bind();
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT,0); // 인덱스를 그리는 함수, indices 개수만큼 전달해야됨.
