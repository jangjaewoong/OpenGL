...
int main()
{
    ...
    GLfloat vertices[] = { // float 형식을 담는 배열. OpenGL의 float는 기존 float와 차지하는 크기가 다르기 때문에 GLfloat를 사용한다.
        -0.5f, -0.5f, 0.0f,
        0.5f, -0.5f, 0.0f,
        0.0f, 0.5f, 0.0f // OpenGL은 NDC(Normalized Device Coordinate)라는 좌표 정규화 방식을 이용하는데 -1~1까지의 값을 window화면 비율에 맞춰 재설정 해준다.
    };
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        glDrawArrays(GL_TRIANGLES, 0, 3); // 정점을 어떤 도형으로 그릴지,VBO에서 몇번째 정점부터 시작할지, 몇개의 정점을 그릴지.
